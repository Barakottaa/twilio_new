...
const PROXY_WEBHOOK_URL = 'http://localhost:8080/api/bird/webhook';
const PHONE_NUMBER = '+201016666348';

async function testWebhookThroughProxy() {
  console.log('🧪 Testing Webhook Through Proxy');
  console.log('📱 Phone:', PHONE_NUMBER);
  console.log('🌐 Proxy Webhook URL:', PROXY_WEBHOOK_URL);
  ...
  try {
    ...
    console.log('📤 Sending webhook through proxy...');
    ...
    const response = await axios.post(PROXY_WEBHOOK_URL, webhookPayload, {
      headers: {
    ...
    if (response.data.success && response.data.processed) {
      console.log('🎉 PDF processing triggered through proxy!');
    ...
    if (error.response?.status === 404) {
      console.log('💡 404 Error - Proxy routing issue');
    }
  }
}

testWebhookThroughProxy();
