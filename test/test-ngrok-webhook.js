...
const NGROK_WEBHOOK_URL = 'https://undegenerated-nonviscidly-marylou.ngrok-free.dev/api/bird/webhook';
const PHONE_NUMBER = '+201016666348';

async function testNgrokWebhook() {
  console.log('🧪 Testing Ngrok Webhook URL');
  ...
  try {
    ...
    const response = await axios.post(NGROK_WEBHOOK_URL, webhookPayload, {
      headers: {
    ...
    if (response.data.success && response.data.processed) {
      console.log('🎉 PDF processing triggered via ngrok!');
