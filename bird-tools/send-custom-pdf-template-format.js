...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith('--phone=')) {
      ...
      config.sendTemplateFirst = false;
...
function showHelp() {
  console.log(`
🚀 Custom PDF in Template Format

📋 Usage:
   node send-custom-pdf-template-format.js --phone=+1234567890 [options]

📋 What it does:
   1. Sends template message first (opens 24-hour window)
   2. Uploads your custom PDF from D:\\Results
   3. Sends your PDF in the exact same format as the template
   4. Uses the same Arabic text and WhatsApp link as the template
...
📋 Options:
   ...
   --no-template           Skip template message (not recommended)
   --help, -h              Show this help

📋 Examples:
   # Send custom PDF in template format
   node send-custom-pdf-template-format.js --phone=+201557000970 --file="D:\\Results\\report.pdf"
   
   # Send all PDFs from folder
   node send-custom-pdf-template-format.js --phone=+201557000970
`);
}
...
// Send custom PDF in template format (same structure as template)
async function sendCustomPDFInTemplateFormat(phone, mediaUrl, fileName, contentType) {
  const url = `https://api.bird.com/workspaces/${WORKSPACE_ID}/channels/${CHANNEL_ID}/messages`;
  ...
  const payload = {
    ...
    body: {
      type: "file",
      file: {
        // Same Arabic text as the template
  ...
  try {
    ...
    throw new Error(`Custom PDF send failed: ${error.response?.data?.message || error.message}`);
  }
}
...
async function processFile(filePath, phone, contentType, sendTemplateFirst) {
  ...
  try {
    ...
    if (sendTemplateFirst) {
      ...
      console.log('⏱️  Waiting 2 seconds before sending custom PDF...');
    ...
    // Step 4: Send custom PDF in template format
    console.log('📤 Sending custom PDF in template format...');
    const result = await sendCustomPDFInTemplateFormat(phone, mediaUrl, fileName, contentType);
    console.log(`✅ Custom PDF sent: ${fileName}`);
    console.log(`   📨 Message ID: ${result.id}`);
    console.log(`   📊 Status: ${result.status}`);
    console.log(`   📝 Format: Same as template (Arabic text + WhatsApp link)`);
...
// Main function
async function main() {
  console.log('🚀 Custom PDF in Template Format\n');
  ...
  console.log(`📋 Template First: ${config.sendTemplateFirst ? 'Yes (recommended)' : 'No'}`);
  ...
  for (const filePath of files) {
    const result = await processFile(filePath, config.phone, config.contentType, config.sendTemplateFirst);
    if (result) {
      results.push(result);
    }
  }
  ...
  console.log(`\n💡 Your custom PDFs were sent in the exact same format as the template:`);
  console.log(`   📝 Same Arabic text`);
  console.log(`   🔗 Same WhatsApp link`);
  console.log(`   📄 Your custom PDF file`);
}
