...
class PdfToImageService {
  ...
  /**
   * Send image to Bird API
   * @param {string} phoneNumber - Recipient phone number
   * @param {string} imagePath - Path to image file
   * @param {string} caption - Image caption
   * @returns {Promise<Object>} - Response object
   */
  async sendImageToBird(phoneNumber, imagePath, caption = '') {
    try {
      console.log('🕊️ Sending image to Bird:', { phoneNumber, imagePath, caption });
    ...
    try {
      ...
      if (mediaType === 'application/pdf' && mediaUrl && contact) {
        ...
        try {
          ...
          // Send each image back to the contact
          const results = [];
          for (let i = 0; i < imageFiles.length; i++) {
            const imagePath = imageFiles[i];
            
            // Optimize image for WhatsApp
            const optimizedPath = await this.optimizeImageForWhatsApp(imagePath);
            ...
            const result = await this.sendImageToBird(contact, optimizedPath, caption);
            results.push(result);
          ...
          console.log('✅ All images sent successfully:', results);
